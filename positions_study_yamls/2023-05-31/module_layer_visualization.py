...
def plot_with_globals(data_arr, outname, run_labels, layer_names, glob_data1, glob_data2, y_axis):
    ...
    if y_axis == 'Rx' or y_axis == 'Tx':
        ...
        for runs in range(total_num_runs):
            correct_x_order = [x_means[runs][iter] for iter in correct_order]
            plt.plot(z_positions, x_means[runs], ls='', marker=markers[runs], c=colors[runs], markersize=10, label=f'{run_labels[runs]}')
            if plt.grid(True):
                plt.grid()
            plt.legend(loc='best')
...
plot_with_globals(tx_xydiff, 'glob_z_vs_local_', labels_xy, layers, glob_z_xydiff, glob_x_xydiff, 'Tx')
plot_with_globals(rx_xydiff, 'glob_z_vs_local_', labels_xy, layers, glob_z_xydiff, glob_x_xydiff, 'Rx')
plot_with_globals(tz_xydiff, 'glob_z_vs_local_', labels_xy, layers, glob_z_xydiff, glob_x_xydiff, 'Tz')
